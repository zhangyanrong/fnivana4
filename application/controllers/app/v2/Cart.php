...
class Cart extends CI_Controller {
	 ...
	 *
	 * @apiSampleRequest /app/v2?service=cart.add&source=app
	 */
	public function add() {
...
// 		$pmt_ids			= explode(',', $this->input->post('pmt_id'));
	 ...
	 * @apiParam {String} [area_adcode] 三级地区编码
	 *
	 * @apiSampleRequest /app/v2?service=cart.count&source=app
	 */
	 ...
	 * @apiParam {String} [area_adcode] 三级地区编码
	 *
	 * @apiSampleRequest /app/v2?service=cart.get&source=app
	 */
	public function get() {
		$this->fruit_log->track('INFO', date('H:i:s') . ';controller-method:' . __METHOD__ . '||start||request_id:' . $this->request_id);
	 ...
	 *
	 * @apiSampleRequest /app/v2?service=cart.remove&source=app
	 */
...
// 		$service_cart_query['item_id']	= $this->input->post('item_id');//需要删除的商品，多个用逗号分隔
		...
		foreach ($item_id_arr as $item_id) {
			...
			if ($response_result->info->http_code == 200 || $response_result->info->http_code == 201) {
				...
				$api_gateway_response['code']	= '300';
				if (isset($service_cart_response['msg'])) {
        				$api_gateway_response['msg']	= $service_cart_response['msg'];
				} elseif (isset($service_cart_response['error'])) {
        				$api_gateway_response['msg']	= $service_cart_response['error'];
				} else {
        				$api_gateway_response['msg']	= '购物车删除失败';
				}
			}
			$this->response = $api_gateway_response;
		}
	 ...
	 * @apiParam {String} [area_adcode] 三级地区编码
	 *
	 * @apiSampleRequest /app/v2?service=cart.clear&source=app
	 */
	 ...
	 *
	 * @apiSampleRequest /app/v2?service=cart.increase&source=app
	 */
	 ...
	 * @apiParam {Number} item_id Item ID
	 *
	 * @apiSampleRequest /app/v2?service=cart.decrease&source=app
	 */
	 ...
	 * @apiParam {Number} item_id Item ID
	 *
	 * @apiSampleRequest /app/v2?service=cart.select&source=app
	 */
	 ...
	 * @apiParam {Number} item_id Item ID
	 *
	 * @apiSampleRequest /app/v2?service=cart.unselect&source=app
	 */
	 ...
	 * @apiParam {String} [area_adcode] 三级地区编码
	 *
	 * @apiSampleRequest /app/v2?service=cart.selectall&source=app
	 */
...
// 				$api_gateway_response = $service_cart_response;
	 ...
	 * @apiParam {String} [area_adcode] 三级地区编码
	 *
	 * @apiSampleRequest /app/v2?service=cart.unselectall&source=app
	 */
...
// 				$api_gateway_response = $service_cart_response;
	 ...
	 * @apiParam {String} [area_adcode] 三级地区编码
	 *
	 * @apiSampleRequest /app/v2?service=cart.merge&source=app
	 */
...
// 				$service_cart_query['item_id'] = $item_id;
	 ...
	 *
	 * @apiSampleRequest /app/v2?service=cart.exchange&source=app
	 */
	 ...
	 * @apiParam {String} [area_adcode] 三级地区编码
	 *
	 * @apiSampleRequest /app/v2?service=cart.exchangeBlock&source=app
	 */
	public function exchangeBlock() {
	    ...
	    if ($response_result->info->http_code == 200 || $response_result->info->http_code == 201) {
	        ...
	        foreach ($filter_product['products'] as $k => $v) {
	            if ($v['valid'] == false && $v['error'] == '该商品已缺货') $filter_product['products'][$k]['error_type'] = 'out_of_stock';
	            if ($v['valid'] == false) array_push($response_product, $filter_product['products'][$k]);
	            else array_unshift($response_product, $filter_product['products'][$k]);
	        }
	        $api_gateway_response['code']           = '200';
	        $api_gateway_response['msg']            = '换购列表加载成功';
	        $api_gateway_response['products']       = $response_product;
	    } else {
	        $api_gateway_response['code']   = '300';
	        $api_gateway_response['msg']    = $service_cart_response['msg'];
	    }
	 ...
	 *
	 * @apiSampleRequest /app/v2?service=cart.validate&source=app
	 */
	 ...
	 *
	 * @apiSampleRequest /app/v2?service=cart.mark&source=app
	 */
	public function mark() {
		...
		try {
			...
			foreach ($product_ids as $k => $v) {
				...
				$service_cart_response = $this->api_process->process($params);
				$response_result = $this->api_process->get('result');
				if ($response_result->info->http_code == 200 || $response_result->info->http_code == 201) $mark_success = true;
			}
	 ...
	 *
	 * @apiSampleRequest /app/v2?service=cart.promotion&source=app
	 */
	 ...
	 *
	 * @apiSampleRequest /app/v2?service=cart.add_on&source=app
	 */
	public function add_on() {
		...
		if ($response_result->info->http_code == 200 || $response_result->info->http_code == 201) {
			...
			$service_response['promotionMsg']    =   $service_cart_response['promotionMsg'];
			$key = 0;
			foreach ($service_cart_response['products'] as $k => $v) {
			    if ($v['product_id'] == '' || $v['valid'] == false) {//商品号为空时为底层异常数据，需要拦截; valid为false时需要拦截数据;
					continue;
				}
				$service_response['products'][$key] = $v;
				$service_response['products'][$key]['product_name']	= $v['name'];
				$service_response['products'][$key]['photo']			= $v['product']['photo'];
				$service_response['products'][$key]['stock']			= 1;//@TODO
				$service_response['products'][$key]['id']			= $v['product_id'];
				$service_response['products'][$key]['unit']			= $v['sku']['unit'];
				$service_response['products'][$key]['volume']		= $v['sku']['volume'];
				$service_response['products'][$key]['setAlert']		= -1;//@TODO
// 				if (isset($v['delivery_tag'])) $service_response['products'][$key]['deliverType']= $v['delivery_tag'];
				if (isset($v['delivery_type'])) $service_response['products'][$key]['deliverType']= $v['delivery_type'];
				$service_response['products'][$key]['product_desc']	= '';//@TODO,$v['product_desc']
				$key++;
			}
	 ...
	 *
	 * @apiSampleRequest /app/v2?service=cart.virtualget&source=app
	 */
	...
	private function filter($products) {
		...
		foreach ($products['products'] as $k => $v) {
			...
			$products['products'][$k]['desc']            = isset($v['product']['product_desc']) ? $v['product']['product_desc'] : '';
			...
			$products['products'][$k]['whitephoto']		= isset($v['product']['whitephoto']) ? $v['product']['whitephoto'] : '';
			if (strtoupper($this->input->get_post('platform')) != 'IOS' && $v['product']['has_webp']) {
				...
				$products['products'][$k]['whitephoto']		= str_replace('.jpg', '.webp', $products['products'][$k]['whitephoto']);//@TODO,最好放外面当参数传递
			}
			...
			$products['products'][$k]['deliveryType']    = isset($v['delivery_type']) ? $v['delivery_type'] : '';//根据IOS邓斌要求 需要将 delivery_type => deliveryType
